...
from hyperframe.frame import Frame, SettingsFrame, PriorityFrame, HeadersFrame
from hyperframe.frame import WindowUpdateFrame, GoAwayFrame, ContinuationFrame, RstStreamFrame
...
class Http2StreamState:
    ...
    # flag set or when either peer sends a RST_STREAM frame.
    ...
    def is_allowed_frame(self, frame: Frame):
        ...
        if self.state == Http2StreamState.HALF_CLOSED:
            if self.header_status == Http2Stream.DONE:
                ...
                # If an endpoint receives additional frames, other than WINDOW_UPDATE, PRIORITY, or RST_STREAM,
                # for a stream that is in this state, it MUST respond with a stream error (Section 5.4.2)
...
class Http2Settings:
    ...
    def check_settings_if_throw(self, settings: dict[int, int]) -> None:
        ...
        if v := settings.get(Http2Settings.MAX_FRAME_SIZE):
            ...
            # the maximum allowed frame size (224-1 or 16,777,215 octets), inclusive.
